...
def main():
    ...
    if flag_log:
        if flag_manual_implement:
            foutput = open("SGD_custom_implement_results.log", "w")
            foutput.write('SDG using custom implementation' + '\n')
        else:
            foutput = open("SGD_pytorch_built-in_results.log", "w")
            foutput.write('SDG using torch built-in' + '\n')
